...
import logging
import sys
from rename_FASTQ import rename_FASTQ,parse_FASTQ
##########
...
def confirm_md5sum(filename,bucket_name,your_key,credential_json):
	...
	return validation
################
### rename files metadata ---- deprecated
...
# for each file in input JSON, create data + expose temporary AWS creds
	...
	# download data from BSSH and rename FILE
		foi_id  = create_data(my_api_key,project_name, rename_FASTQ(foi), "FILE", folder_id=folder_id)
		foi_md5_id = create_data(my_api_key,project_name,rename_FASTQ(foi) +".md5sum", "FILE", folder_id=folder_id)
		download_data_from_url(download_url, output_name=rename_FASTQ(foi))
		foi = rename_FASTQ(foi)
		foi_md5 = f"{foi}.md5sum"
